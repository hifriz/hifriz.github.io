        ...
        function(data) {
            ...
            for (key in allDates) {
                if (key.indexOf(inputDate) != -1) {
                    ...
                    output
                        .push({ timePoint: d3.time.format("%Y-%m-%dT%H:%M:%S.%L").parse(key), value: allDates[key] });
...
function drawLOB(inputDate, selectedStocks) {
    ...
    selectedStocks.forEach(
        function(element) {
            ...
            stockLobViz.appendChild(stockLobVizText);
...
function drawLobDetail(name, inputDate = "2019-01-03") {
    let division = document.getElementById(name);
    let groups = ["Volume", "Bid/Ask"];
    ...
    setTimeout(() => {
        ...
        let x = d3.scaleTime()
            ...
            .range([0, width_num]);
        svg.append("g")
            .attr("transform", "translate(0," + height_num + ")")
